    ...
    <footer className={`py-12 px-4 border-t ${dark ? 'border-white/[0.06]' : 'border-gray-100'}`}>
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
          <span className="font-display font-extrabold text-xl grad-text select-none">Samira Aboutarik</span>

          <div className="flex flex-wrap justify-center gap-1">
            {NAV_LINKS.map(({ id, label }) => (
              <Link key={id} to={id} smooth duration={700} offset={-80}
                className={`text-xs font-medium px-3 py-1.5 rounded-lg cursor-pointer transition-colors ${dark ? 'text-gray-500 hover:text-gray-200 hover:bg-white/5' : 'text-gray-400 hover:text-gray-900 hover:bg-gray-100'}`}>
                {label}
              </Link>
            ))}
          </div>
        ...
        <div className={`flex flex-col md:flex-row items-center justify-between gap-2 text-xs ${dark ? 'text-gray-600' : 'text-gray-400'}`}>
          <p>© 2025 Samira Aboutarik · All rights reserved</p>
          <p className="flex items-center gap-1.5">
            Designed & built with <HiHeart size={12} className="text-red-400" /> from{' '}
