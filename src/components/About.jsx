...
const INFO = [
  { Icon: HiMapPin,      label: 'Location',     value: 'Agadir, Morocco' },
  { Icon: HiAcademicCap, label: 'Education',     value: 'OFPPT DEVOWFS' },
  { Icon: HiBriefcase,   label: 'Availability',  value: 'Internship / Full-time' },
  { Icon: HiLanguage,    label: 'Languages',     value: 'Arabic · French · English' },
]
    ...
    <section id="about" className="py-28 px-4 max-w-6xl mx-auto" ref={ref}>
      <div className="grid md:grid-cols-2 gap-20 items-center">
        ...
        <motion.div variants={fadeL} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="flex justify-center">
          <div className="relative select-none">
            ...
            <motion.div animate={{ y: [0,-6,0] }} transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
              ...
              <div className={`text-[11px] font-medium ${dark ? 'text-gray-400' : 'text-gray-500'}`}>years of training</div>
            </motion.div>
            <motion.div animate={{ y: [0,-6,0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
              ...
              <div className={`text-[11px] font-medium ${dark ? 'text-gray-400' : 'text-gray-500'}`}>apps shipped</div>
            </motion.div>
          </div>
        </motion.div>
        ...
        <motion.div variants={fadeR} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="space-y-7">
          <div>
            <p className="text-xs font-bold grad-text uppercase tracking-[0.25em] mb-3">Who am I?</p>
            <h2 className="font-display font-bold text-4xl md:text-5xl leading-tight">
              Full Stack Developer{' '}
              <span className="grad-text">ready for your next challenge</span>
            </h2>
          </div>
          <p className={`leading-relaxed text-base ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
            Trained at <strong className={dark ? 'text-white font-semibold' : 'text-gray-900 font-semibold'}>OFPPT in Full Stack Web Development</strong>, I build end-to-end digital solutions: solid backend architecture with Laravel & PHP, modern interfaces with React & Tailwind, and optimized databases with MySQL & MongoDB.
          </p>
          <p className={`leading-relaxed text-base ${dark ? 'text-gray-400' : 'text-gray-600'}`}>
            What sets me apart: I don't just deliver code — I deliver products. Every project I take on is designed for the end user, optimized for performance, and built to scale.
          </p>

          <div className="grid grid-cols-2 gap-3">
            {INFO.map(({ Icon, label, value }) => (
              <div key={label} className={`flex items-center gap-3 p-3.5 rounded-xl transition-all duration-300 ${
                dark ? 'bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06]' : 'bg-white border border-gray-100 shadow-sm hover:shadow-md'
              }`}>
