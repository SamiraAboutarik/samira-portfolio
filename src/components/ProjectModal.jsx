        ...
        <motion.div initial={{ opacity: 0, scale: 0.88, y: 28 }} animate={{ opacity: 1, scale: 1, y: 0 }}
          ...
          <div className="p-8">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 id="project-modal-title" className="font-display font-bold text-2xl mb-1">{project.title}</h3>
                <p className={`text-sm ${dark ? 'text-gray-400' : 'text-gray-500'}`}>{project.description}</p>
              </div>
              <motion.button aria-label="Fermer le modal" whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }} onClick={onClose}
            ...
            {project.image ? (
              ...
              <div className={`mb-6 h-36 rounded-2xl bg-gradient-to-br ${project.color} flex flex-col items-center justify-center gap-2`}>
                <HiExclamationTriangle size={24} className="text-white/60" />
                <span className="text-white/80 text-sm font-semibold">{project.title}</span>
              </div>
            )}
