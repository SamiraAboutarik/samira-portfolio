      ...
      >
        <motion.div
          ...
          className={`rounded-3xl max-w-lg w-full overflow-hidden ${
            dark ? 'bg-[#0c0c18] border border-white/10' : 'bg-white border border-gray-200 shadow-2xl'
          }`}
          ...
          <div className="p-8">
            ...
            <div className="flex items-start justify-between mb-6">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={onClose}
                className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ml-4 shrink-0 ${
                  dark ? 'bg-white/8 hover:bg-white/15 text-gray-400 hover:text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-500'
                }`}
              >
                <HiXMark size={18} />
              </motion.button>
            ...
            <div className="space-y-3 mb-6">
              <div className={`rounded-2xl p-4 flex gap-3 ${
                dark ? 'bg-red-500/5 border border-red-500/12' : 'bg-red-50 border border-red-100'
              }`}>
                <HiExclamationTriangle size={18} className="text-red-400 shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-bold text-red-400 uppercase tracking-wider mb-1">Problème</p>
                  <p className={`text-sm leading-relaxed ${dark ? 'text-gray-300' : 'text-gray-600'}`}>{project.problem}</p>
                </div>
              </div>
              <div className={`rounded-2xl p-4 flex gap-3 ${
                dark ? 'bg-green-500/5 border border-green-500/12' : 'bg-green-50 border border-green-100'
              }`}>
                <HiCheckCircle size={18} className="text-green-400 shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-bold text-green-400 uppercase tracking-wider mb-1">Solution</p>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border ${
                        dark ? 'bg-white/5 border-white/8 text-gray-300' : 'bg-gray-50 border-gray-200 text-gray-700'
                      }`}
                    >
            ...
            <div className="flex gap-3">
              <motion.a
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold text-sm transition-colors ${
                  dark ? 'bg-white/8 hover:bg-white/15 text-gray-200' : 'bg-gray-900 hover:bg-gray-800 text-white'
                }`}
              >
                <SiGithub size={16} />
                GitHub
