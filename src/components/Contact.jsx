...
const CONTACT_INFO = [
  ...
  { Icon: HiMapPin,   label: 'Location',  value: 'Agadir, Maroc 🇲🇦',                href: '#',                                 color: 'bg-red-500/10 text-red-400' },
]
...
export default function Contact({ dark }) {
  ...
  const fieldClass = (name) => `
    ...
    ${dark
      ? 'bg-white/5 border border-white/10 text-white placeholder-gray-600 hover:border-white/20'
      : 'bg-gray-50 border border-gray-200 text-gray-900 placeholder-gray-400 hover:border-gray-300 focus:bg-white'
    }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className={`mt-4 max-w-lg mx-auto text-base ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
            Open to stage, CDI, freelance et collaborations. Je réponds sous 24h.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          >
            {CONTACT_INFO.map(({ Icon, label, value, href, color }) => (
              ...
              >
                ...
                <HiMapPin size={14} className={`ml-auto opacity-0 group-hover:opacity-100 transition-opacity grad-text`} />
              </motion.a>
            ))}
            ...
            <div className="grad-btn p-6 rounded-2xl text-white text-center shadow-lg shadow-purple-500/20 mt-2">
              ...
              <p className="font-display font-bold text-xl mb-1">Disponible dès maintenant</p>
              <p className="text-white/75 text-sm">Stage · CDI · Freelance</p>
            </div>
          </motion.div>
          ...
          <motion.form
            ...
            className={`rounded-2xl p-7 border space-y-5 ${
              dark ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-white border-gray-100 shadow-lg'
            }`}
          >
            <div className="grid grid-cols-2 gap-4">
              {[
                { id: 'name',  label: 'Nom *',   placeholder: 'Votre nom',         type: 'text' },
            ...
            <div>
              <label className={`text-xs font-semibold mb-2 block ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Sujet</label>
              <input
                type="text"
